...
const RoomCard = ({ roomName, activeUsers, onJoin }: RoomCardProps) => {
  return (
    <Card className="group hover:shadow-lg transition-all duration-300 bg-gradient-card border-border/50 animate-fade-in hover:border-primary/30 hover:-translate-y-1 cursor-pointer">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <CardTitle className="text-lg font-bold truncate mb-1">{roomName}</CardTitle>
              <CardDescription className="flex items-center gap-1.5 text-sm">
                <Users className="w-4 h-4" />
                <span>{activeUsers.length} {activeUsers.length === 1 ? 'participant' : 'participants'}</span>
              </CardDescription>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="flex -space-x-2 flex-1">
            ...
            {activeUsers.length > 4 && (
              <div className="w-8 h-8 rounded-full border-2 border-background bg-muted flex items-center justify-center">
                <span className="text-xs font-semibold text-muted-foreground">+{activeUsers.length - 4}</span>
              </div>
            )}
          </div>
        </div>
        <Button 
          onClick={onJoin}
          className="w-full bg-gradient-primary hover:opacity-90 transition-all duration-300 shadow-medium hover:shadow-lg hover:scale-[1.02] font-semibold group/btn"
        >
          Join Room
